...
class XpressbeesPrepaidCourierController extends Controller
{
    ...
    public function __construct($order_ids = array() , $courier_id = 0 , $company_id = 0,$courier_settings=array()){
        ...
        $this->api_url = 'https://shipment.xpressbees.com';        
        ...
        foreach ($this->order_ids as $order_id) {
            ...
            if(!empty($order_info['order_totals'])){
                foreach($order_info['order_totals'] as $order_totals){
                    ...
                    if($order_totals['code']=='sub_total'){
                        $sub_total = $order_totals['value'];
                    }
            ...
            $packageInfo = [
                ...
                "is_rto_different" =>($this->pickup_address->id==$this->return_address->id)?"yes":"no",
                "rto" => [
                    "warehouse_name" => $this->return_address->location_title,
                    "name" => $this->return_address->contact_person_name,
                    "address" => $this->return_address->address,
                    "address_2" => $this->return_address->landmark,
                ...
                "courier_id" =>$this->xb_courier_id,
                "collectable_amount" => $order_info['payment_mode'] === 'prepaid' ? 0:$order_info['order_total'],
            ];    
            if (ShipmentInfo::where('order_id', $order_id)->exists()) {
                $this->result['error'][$order_info['vendor_order_number']] = 'Tracking number already assigned';
                continue;
            }      
            $apiUrl = $this->api_url.'/api/shipments2';
            $headers = [
                    ...
                    DB::transaction(function () use (
                        ...
                        * -------------------------------- */
                        ...
                        app(SellerWalletService::class)->applyFreight([
                            ...
                            'courier_id'      => $this->courier_id,
                            'courier_code'    => 'xpressbees_prepaid',
   ...
   public function authentication(){
        $company_id = $this->company_id;
        $token = Cache::get("api_auth_token_xpressbees_prepaid_{$this->courier_id}_{$company_id}");        
        $apiUrl = $this->api_url.'/api/users/login';       
        ...
        if (!$token) {
            ...
            if($token){
                Cache::put("api_auth_token_xpressbees_prepaid_{$this->courier_id}_{$company_id}", $token, now()->addMinutes(30));
            }
            if($this->action == 'print_response'){
    ...
    public function trackShipment($order_id,$tracking_number){
        ...
        $url = $this->api_url.'/api/shipments2/track/'.$tracking_number;
        $headers = [
            'Authorization' => 'Bearer ' . $token,
            'Content-Type' => 'application/json',
        ];
        ...
        $message = $response['message']??'';
      //return $shipmentData;
        if($status===true && $shipmentData){
            $shipmentData = $response['history']??[];
            $scansdata['courier_id'] = $this->courier_id;  
            $scansdata['tracking_number'] = $tracking_number;  
            $scansdata['origin'] = '';                 
            $scansdata['destination'] = '';
            $scansdata['pickup_date'] =  (!empty($shipmentData['created']))?date('Y-m-d H:i:s',strtotime($shipmentData['created'])):''; 
            ...
            foreach($scans as $ScanDetail){
                ...
                $trackingHistories['date'] =(!empty($ScanDetail['event_time']))?date('Y-m-d H:i:s',strtotime($ScanDetail['event_time'])):''; 
    ...
    public function cancelShipments(){
        $url = $this->api_url.'/api/shipments2/cancel';
        if ($this->order_ids) {
            try {
    ...
    public function calculateShipping(){
        ...
        foreach ($this->order_ids as $order_id) {
            ...
            $package_dead_weight = ($order_info['package_dead_weight'])?$order_info['package_dead_weight']:0.05;
            $package_dead_weight = $package_dead_weight*1000;
            ...
            if($payment_mode==='prepaid'){
                ...
                $collectable_amount = $order_info['order_total'];
                $payment_mode = 'cod';
            }
            
            $url = $this->api_url.'/api/courier/serviceability';
            $headers = [
                'Authorization' => 'Bearer ' . $token,
                'Content-Type' => 'application/json',
            ];
            $post_fields = [
                "origin" => $this->pickup_address->zipcode,
                ...
                "height" => $order_info['package_heigth'] ?? 10,
            ];
            ...
            if($response['status']===true && !empty($detail)){
                $res = $detail[0]??[];
                $total_amount=$res['"total_charges']??0;
                $this->result['success'] = true;
